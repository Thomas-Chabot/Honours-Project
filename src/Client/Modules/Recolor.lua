...
function Recolor.SetTarget(target)
    ...
    -- exit conditions: no target OR same target
    if not target or target == currentTarget.Target then
        return
    end
